...
export class FullPage {
	...
	setEvents() {
		...
		// КАСТОМНЫЕ КЛИКИ И СОБЫТИЯ ======================

		const btnNextSect00 = document.querySelector('.section-00__btn-next');
		const btnScrollSect00 = document.querySelector('.section-00__scroll');
		if (btnNextSect00) {
			btnNextSect00.addEventListener('click', () => {
				this.switchingSection(1);
			});
		}
		if (btnScrollSect00) {
			btnScrollSect00.addEventListener('click', () => {
				this.switchingSection(1);
			});
		}
	...
	// setEvents() {
	// 	// Опции для пассивных событий
	// 	const passiveOptions = { passive: true };
	
	// 	// Инициализация событий
	// 	const initEvents = () => {
	// 		this.wrapper.addEventListener('wheel', this.events.wheel, passiveOptions);
	// 		this.wrapper.addEventListener('touchstart', this.events.touchdown, passiveOptions);
	// 	};
	
	// 	// Отключение событий
	// 	const disableEvents = () => {
	// 		this.wrapper.removeEventListener('wheel', this.events.wheel, passiveOptions);
	// 		this.wrapper.removeEventListener('touchstart', this.events.touchdown, passiveOptions);
	// 	};
	
	// 	// Проверка на наличие класса section-00 и активного класса
	// 	const section00 = document.querySelector('.section-00');
	// 	const checkSection00Active = () => {
	// 		if (section00) {
	// 			if (section00.classList.contains('_active')) {
	// 				initEvents();
	// 			} else {
	// 				disableEvents();
	// 			}
	...
	// 	// Первоначальная проверка и установка событий
	// 	checkSection00Active();
	
	// 	// Наблюдатель за изменением классов
	// 	const observer = new MutationObserver(mutations => {
	// 		mutations.forEach(mutation => {
	// 			if (mutation.attributeName === 'class') {
	// 				const target = mutation.target;
	// 				if (target.classList.contains('section-00')) {
	// 					checkSection00Active();
	// 				}
	// 			}
	// 		});
	...
	// 	// Начало наблюдения за изменениями классов у элемента section-00
	// 	if (section00) {
	// 		observer.observe(section00, {
	// 			attributes: true
	// 		});
	// 	}
	
	// 	// Инициализация кастомных кликов и событий
	// 	const btnNextSect00 = document.querySelector('.section-00__btn-next');
	// 	const btnScrollSect00 = document.querySelector('.section-00__scroll');
	// 	if (btnNextSect00) {
	// 		btnNextSect00.addEventListener('click', () => {
	...
	// 	// Инициализация события клика по булетам
	// 	if (this.options.bullets && this.bulletsWrapper) {
	// 		this.bulletsWrapper.addEventListener('click', this.events.click);
	// 	}
	// }
